 ...
 *
 * Este controlador tiene dos responsabilidades principales segun el metodo
 * HTTP:
 * </p>
 * 
 * POST /pedido - Crea un nuevo pedido (checkout del carrito) o actualiza
 * el estado logistico de un pedido existente (Preparando, Enviado, Entregado,
 * Cancelado).</li>
 ...
 * 
 *
 * Implementa validaciones de seguridad estrictas: verifica la sesion activa
 * (401),
 * el nivel jerarquico del usuario para cambios de estado (403) y el formato de
 * los
...
public class PedidoController extends HttpServlet {
     ...
     * actua como un guardia de trafico bidireccional dependiendo del contenido
     * enviado:
     * 
     * ruta a: si detecta "accion = cambiar_estado", asume que es un empleado
     * gestionando envios.
     * ruta b: si detecta arreglos de productos, asume que es un cliente haciendo
     * "checkout".
     * 
     * en ambos casos implementa rigurosas barreras de seguridad (401 y 403) para
     * evitar hackeos.
     * 
     * @param request  httpservletrequest: el paquete http enviado por javascript
     *                 (con data o parametros).
     * @param response httpservletresponse: el objeto usado para contestarle al
     *                 navegador con codigos http.
     */
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // condicional de seguridad: si no hay cookies activas, expulsa la peticion con
        // un error 401.
        ...
        // validacion de accion: ahora coincide con el nombre enviado desde
        // pedidoservice.js
        if ("actualizar_estado".equals(accion)) {

            // barrera de privilegios: solo permitimos el paso a administradores,
            ...
            // extraemos el id usando el nombre de parametro correcto enviado por el
            // frontend
            int idPedido = Integer.parseInt(request.getParameter("id"));
            String nuevoEstado = request.getParameter("estado");
            String motivo = request.getParameter("motivo"); // capturamos la razon enviada desde el modal.

            // validacion de seguridad: el cliente solo puede cancelar si el pedido esta
            // pendiente.
            ...
            // condicional de respuesta: si el dao devulve true responde 200, sino error
            // 500.
            if (exito) {
        ...
        // =========================================================================
        // atrapamos arreglos completos generados por javascript. ejemplo: ids = [2, 5,
        // 8].
        ...
        // la regex acepta numeros, guiones, espacios y '+' (igual que el frontend) de 4
        // a 25 caracteres.
        // esto previene nullpointerexception y es consistente con la validacion del
        // pedidoService.js
        if (idsProductos == null || idsProductos.length == 0 || idMetodoStr == null || cuenta == null
                || !cuenta.matches("^[0-9\\-\\s\\+]{4,25}$")) {
            // sc_bad_request (400): el servidor rechaza la peticion porque faltan datos
            // clave del carrito.
        ...
        // recorremos el carrito para convertir el texto (string) en numeros matematicos
        // (int/double).
        // iteracion clasica: recorre la longitud del arreglo idsproductos que vino por
        // http.
        for (int i = 0; i < idsProductos.length; i++) {
            ...
            // para evitar que el cliente altere los precios totalizados usando f12
        ...
        // correccion: obtenemos el id del carrito activo directamente desde la base de
        // datos.
        // antes se esperaba que javascript lo enviara, pero nunca lo enviaba (bug
        // critico).
        // carritoDAO.obtenerOCrearCarrito busca el carrito activo del cliente. si no
        // existe, lo crea.
        ...
        // si el cliente no tiene direccion registrada, enviamos 400 para que el
        // frontend lo maneje.
        ...
        // incluye el nuevo parametro idDireccion que antes no se enviaba y causaba el
        // 500
        ...
        if (exito) {
            ...
            // sc_internal_server_error (500): fallo critico (ej. no habia stock o error
            // sql).
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
        }
    }
     ...
     * panel del cliente, el dashboard del administrador y la logistica del
     * proveedor.
     * el aislamiento es absoluto.
     * 
     * @param request  httpservletrequest: la peticion get de lectura.
     * @param response httpservletresponse: escribe el arreglo json hacia el
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // condicional de control de acceso: envia peticiones sql diferentes usando el
        // perfil (rol).
        ...
        // validacion de integridad: si el dao responde nulo por error de red, lo
        // convertimos en lista vacia.
        if (lista == null) {
            lista = new ArrayList<>();
        }

        // utilizamos el motor manual (helper) para parsear los arreglos complejos a
        // texto legible para js.
