...
public class PerfilClienteController extends HttpServlet {
         ...
         * se ejecuta cuando la pagina "perfil.html" termina de cargar y hace un
         * fetch().
         * busca en las tablas satelite (direccion, telefono) y devuelve un json al
         * frontend.
         * 
         * @param request  httpservletrequest: peticion inicial get.
         * @param response httpservletresponse: despacha el json stringificado a la
         *                 pagina.
         */
        @Override
        protected void doGet(HttpServletRequest request, HttpServletResponse response)
                ...
                // 1. verificamos que el usuario tenga una sesion valida (que no sea un
                // visitante anonimo)
                HttpSession sesion = request.getSession(false);
                // condicional de seguridad: bloquea a los intrusos devolviendo un error de no
                // autorizacion (401).
                ...
                if (perfil != null) {
                        ...
                        // 3.1 candado: aseguramos que el correo jamas llegue vacio cruzando con la
                        // sesion actual
                        String correoSeguro = "";
                        ...
                        // extraemos y saneamos el nombre y apellido para que no rompan el json si
                        // tienen comillas
                        String nombreSeguro = user.getNombre() != null
                                        ? user.getNombre().replace("\"", "\\\"").replace("\n", " ").replace("\r", "")
                                        : "";
                        String apellidoSeguro = user.getApellido() != null
                                        ? user.getApellido().replace("\"", "\\\"").replace("\n", " ").replace("\r", "")
                                        : "";
                        ...
                        json.append("{")
                                        ...
                                        // reemplazamos los nulls por textos vacios para que javascript no imprima la
                                        // palabra "null"
                                        // se usa un operador ternario en linea (condicion ? verdadero : falso) para
                                        // evitar imprimir "null".
                                        .append("\"direccion\":\"")
                                        .append(perfil.getDireccion() != null
                                                        ? perfil.getDireccion().replace("\"", "\\\"").replace("\n", " ")
                                                                        .replace("\r", "")
                                                        : "")
                                        .append("\",")
                                        .append("\"direccion_detallada\":\"")
                                        .append(perfil.getDireccionDetalle() != null
                                                        ? perfil.getDireccionDetalle().replace("\"", "\\\"")
                                                                        .replace("\n", " ").replace("\r", "")
                                                        : "")
                                        .append("\",")
                                        .append("\"telefono\":\"")
                                        .append(perfil.getTelefono() != null
                                                        ? perfil.getTelefono().replace("\"", "\\\"").replace("\n", " ")
                                                                        .replace("\r", "")
                                                        : "")
                                        .append("\",")
                                        .append("\"telefono_secundario\":\"")
                                        .append(perfil.getTelefonoSecundario() != null
                                                        ? perfil.getTelefonoSecundario().replace("\"", "\\\"")
                                                                        .replace("\n", " ").replace("\r", "")
                                                        : "")
                                        .append("\",")
                                        .append("\"referencia_ubicacion\":\"")
                                        .append(perfil.getReferencia() != null
                                                        ? perfil.getReferencia().replace("\"", "\\\"")
                                                                        .replace("\n", "\\n").replace("\r", "")
                                                        : "")
                                        .append("\",")
                                        .append("\"correo\":\"").append(correoSeguro).append("\"")
                                        .append("}");
                        // Status line 200 indica que la peticion fue exitosa y que hay datos para
                        // mostrar
                        response.setStatus(HttpServletResponse.SC_OK); // 200 OK
                        // Body de la respuesta de vuelta http del usuario
                        response.getWriter().print(json.toString());
                        // En caso que no encontro ningun registro previo
                } else {
                        String nombreSeguro = user.getNombre() != null
                                        ? user.getNombre().replace("\"", "\\\"").replace("\n", " ").replace("\r", "")
                                        : "";
                        String apellidoSeguro = user.getApellido() != null
                                        ? user.getApellido().replace("\"", "\\\"").replace("\n", " ").replace("\r", "")
                                        : "";
                        String correoSeguro = user.getCorreo() != null
                                        ? user.getCorreo().replace("\"", "\\\"").replace("\n", " ").replace("\r", "")
                                        : "";

                        response.setStatus(HttpServletResponse.SC_OK);
                        response.getWriter()
                                        .print("{\"nombre\":\"" + nombreSeguro + "\", \"apellido\":\"" + apellidoSeguro
                                                        + "\", \"correo\":\"" + correoSeguro + "\"}");
                }
        }
         ...
         * recibe los parametros de texto desde javascript y le ordena al dao que haga
         * los insert o update.
         * 
         * @param request  httpservletrequest: objeto que contiene los parametros
         *                 (datos) digitados en el input.
        ...
        protected void doPost(HttpServletRequest request, HttpServletResponse response)
                ...
                // condicional de formato: rechaza la operacion si los campos vitales vienen
                // nulos o con texto vacio.
                if (numeroTelefono == null || direccionPrimaria == null || numeroTelefono.trim().isEmpty()
                                || direccionPrimaria.trim().isEmpty()) {
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST); // 400: faltan datos
                        return;
                }

                // 4. se los pasamos a nuestro dao transaccional para que llene las tablas
                // correspondientes
                clienteDAO dao = new clienteDAO();
                boolean exito = dao.guardarOActualizarPerfil(user.getIdUsuario(), direccionPrimaria, direccionDetalle,
                                numeroTelefono, telefonoSecundario, referencia);

                // 5. respondemos al navegador
                // condicional final: envia una senal http ok solo si la transaccion completa de
                // las 3 tablas salio bien
                if (exito) {
