...
*/
@MultipartConfig(
        // limite de memoria ram antes de guardar el archivo temporalmente en disco duro
        // (1mb).
...
/*
 * @webservlet - mapea este servlet a las direcciones url relacionadas con
 * productos.
 */
...
public class ProductoController extends HttpServlet {
    ...
    // doget: responde a peticiones de tipo lectura para mostrar la lista en el
    // frontend.
    // metodo para procesar solicitudes de lectura. filtra por productos activos
    // para el catalogo
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Capturamos fallos internos (como 'con' siendo null) para que no rompan el
            // servidor
            ...
            // imprime el rastro de la excepcion en la consola para identificar la linea
            // exacta del fallo
            e.printStackTrace();
        }
    }

    // dopost: responde a peticiones para modificar datos (crear, actualizar,
    // borrar).
    // funciona como un despachador que lee la ruta de la url (servletpath) para
    // decidir si debe insertar, actualizar, eliminar o cambiar el estado de un
    // producto.
    // @param request peticion con los datos del formulario (multipart/form-data).
    // @param response objeto para devolver codigos de estado ok (200) o error
    // (400).
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("/insertar".equals(ruta)) {
            ...
            if (idGenerado > 0) {
                ...
                // SC_OK equivale al codigo HTTP 200 (OK). Indica a Javascript que todo salio
                // perfecto.
                response.setStatus(HttpServletResponse.SC_OK);
            } else {
                // SC_BAD_REQUEST equivale al codigo HTTP 400 (Bad Request). Indica que hubo un
                // error al guardar.
    ...
    // metodo auxiliar para procesar el archivo fisico, guardarlo en el servidor y
    // en la bd.
    // gestiona la subida de imagenes transformando el nombre del archivo para
    // evitar espacios.
    // guarda la foto en la carpeta temporal de tomcat y la sincroniza con la
    // carpeta de desarrollo.
    // @param request objeto que contiene la parte binaria del archivo (part).
    // @param idproducto identificador numerico para nombrar el archivo de forma
    // unica.
    private void guardarImagenFisica(HttpServletRequest request, int idProducto) {
        try {
            ...
            if (filePart != null && filePart.getSize() > 0) {
                ...
                // IMPORTANTE: Quitamos los espacios en el nombre de la foto porque rompen la
                // URL en HTML
                ...
                // 1. Guardamos en el servidor temporal (Tomcat) para que cargue inmediatamente
                // en la pagina
                ...
                // 2. magia local dinamica: calculamos la ruta fuente cortando el path de
                // despliegue.
                // cuando ejecutas en tu ide, apppath contiene la carpeta "target" o "build".
                // cortamos esa palabra para retroceder a la raiz de tu proyecto, sin importar
                // la pc.
                if (appPath.contains("target")) {
                    String rutaRaiz = appPath.substring(0, appPath.indexOf("target"));
                    String rutaProyecto = rutaRaiz + "src" + File.separator + "main" + File.separator + "webapp"
                            + File.separator + "src" + File.separator + "img" + File.separator + "productos";
                ...
                // ruta que va a la base de datos (siempre con diagonales normales '/' para la
                // web).
            ...
            // printstacktrace ayuda a ver si el problema es de permisos de carpeta o de
            // tamaño de archivo
            e.printStackTrace();
        }
    }
