...
public class carritoDAO {
     ...
     * este metodo actua como un portero. cuando un cliente intenta guardar
     * productos,
    ...
    public int obtenerOCrearCarrito(int idCliente) {
        ...
        // busca un registro en la tabla carrito filtrando por el dueno y que no este
        // procesado
        ...
        // salvavidas: asegura que el cliente exista en su tabla para evitar errores de
        // integridad
        // usa on duplicate key update para evitar el error de clave primaria duplicada
        // si ya existía
    ...
    public String obtenerCarritoJSON(int idCliente) {
        ...
        try (Connection con = db.conectar(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.out.println("Error al obtener carrito JSON: " + e.getMessage());
        }
     ...
     * @param productoslocal arraylist<detallecarrito>: lista de productos a
     *                       guardar.
    ...
    public boolean sincronizarCarrito(int idCliente, ArrayList<detalleCarrito> productosLocal) {
        ...
        try {
            con = db.conectar();
            // Transaccion de Seguridad: Si borrar o insertar falla a la mitad, MySQL
            // cancela todo el proceso

            // el proceso de limpieza selectiva ocurre aqui: al borrar todo e insertar solo
            // lo que
            // el frontend envio (que ya viene filtrado), los productos que se pagaron
            // desaparecen
            // de la tabla mientras que los no seleccionados se mantienen persistentes.

            // Esto evita que un cliente se quede con un carrito medio vacio por un error de
            // internet.
            ...
            // si viene vacio, simplemente ignora la insercion (dejando el carrito limpio
            // tras el delete anterior).
            if (productosLocal != null && !productosLocal.isEmpty()) {
                ...
                try (PreparedStatement psInsert = con.prepareStatement(sqlInsert)) {
                    ...
                    for (detalleCarrito item : productosLocal) {
                        ...
                        // el error en la linea 169 ocurre si el modelo detalleCarrito no tiene
                        // isSeleccionado()
                        ...
                        // enviaremos un solo paquete con las 10 instrucciones (optimiza velocidad y
                        // memoria)
                        psInsert.addBatch();
                    }
            ...
            // Obligatorio: devolver el autocommit a 'true' para no dañar futuras conexiones
            // en el pool
            try {
                if (con != null) {
