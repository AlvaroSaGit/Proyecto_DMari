...
public class productoDAO {
     ...
     * 
     * @param soloActivos boolean: si es true, filtra solo los que tienen stock y
     *                    estado=1.
    ...
    public ArrayList<producto> listarProductos(boolean soloActivos) {
        ...
        String sql = "SELECT " +
                ...
                // uso de left join: traemos el nombre de la categoria. si el producto quedo sin
                // categoria por algun error, no se ocultara.
                "LEFT JOIN categoria c ON p.id_categoria_fk = c.id_categoria_pk " +
                // nuevos left joins: traemos al proveedor (si tiene) para validar si su cuenta
                // sigue activa
        ...
        try (
            ...
            // iteracion: transforma las filas (records) devueltas por mysql en objetos de
            // java.
            while (rs.next()) {
                ...
                // extraemos la cadena de multiples etiquetas y la convertimos en un arreglo
                // (lista)
                String etiquetasStr = rs.getString("etiquetas_str");
                ArrayList<String> listaTags = new ArrayList<>();
                // condicional anidado con iteracion: deserializa las etiquetas (ej:
                // dulce,regalo) en un array list.
     ...
     * lista el catalogo interno que le pertenece a un solo dueno (sin importar si
     * esta pausado).
    ...
    public ArrayList<producto> listarProductosPorProveedor(int idUsuarioProveedor) {
        ...
        // seleccionamos la informacion basica desde la tabla producto
                ...
                // inner join exige que el producto exista obligatoriamente en la tabla
                // 'proveedor_producto'.
                // como los productos oficiales de dmari no estan en esa tabla puente, el inner
                // join los descarta y los oculta.
     ...
     * pide la creacion de la llave primaria para luego usarla en la insercion de
     * fotos y etiquetas.
     * 
     ...
     * garantiza que el inventario jamas quede huerfano si lo creo una cuenta
     * comercial.
     * 
        ...
        try (Connection con = db.conectar()) {
            ...
            if (idProveedorPk > 0) {
                ...
                try (PreparedStatement psInsert = con.prepareStatement(sqlInsert)) {
                    ...
                    System.out.println(
                            "Enlace creado exitosamente: producto " + idProducto + " -> proveedor " + idProveedorPk);
                    return filas > 0;
                }
            }
        } catch (SQLException e) {
     ...
     * @param idUsuarioProveedor int: nueva cuenta duena (si se pasa 0, queda
     *                           expropiado a dmari).
     ...
     * destruye paulatinamente todas las referencias filiales antes de destruir al
     * padre.
     * 
     ...
     * @param nuevoEstado boolean: true para habilitar ventas, false para
     *                    esconderlo.
    ...
    public boolean actualizarEstado(int id, boolean nuevoEstado) {
        ...
        // gestion automatica de recursos (try-with-resources) para garantizar el cierre
        // de la conexion.
        // connection: instancia de enlace persistente con el gestor de base de datos.
        // preparedstatement: estructura de consulta precompilada que sanitiza las
        ...
        try (Connection con = db.conectar();
            ...
            // executeupdate: instruccion dedicada a operaciones dml (insert, update,
            // delete).
            // retorna el numero entero correspondiente al total de filas modificadas en la
            // tabla.
            int filasAfectadas = ps.executeUpdate();

            // validacion de transaccion: si retorna un valor mayor a cero, la operacion fue
            // exitosa.
            return filasAfectadas > 0;
